"""成片 final_path 字段：JSON { path, duration, size }。"""
...
def parse_final_asset(raw: Any) -> dict | None:
    ...
    if text.startswith("{"):
        try:
            ...
            return {"path": text, "duration": None, "size": None}
...
def build_final_asset(path: Path, *, duration: float, size: int | None = None) -> dict:
