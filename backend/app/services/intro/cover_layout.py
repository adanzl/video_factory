...
def _resolve_cover_subject(subject: str) -> str:
    ...
    彻底消除所有地图/世界地图/地球仪相关描述，避免和约束词冲突。"""
    ...
    system = (
        "你是一个封面提示词优化器。请将输入改写为不包含任何地图/世界地图/地球仪元素的版本："
        "1. 删除'世界地图''地球仪''地图背景'等词，用'深色插画背景''俯视场景''俯瞰视角'等替代"
        "2. 删除'地图上标注''虚线轮廓''国界线'等地图特征描述，改用非地图的视觉表达"
        "3. 保留原句的地理位置、颜色、主体（船/海峡等）、风格、构图、光照等所有其他细节"
        "4. 改写后整体不应呈现地图的画面感"
        "仅输出改写后的文本，不要额外解释。"
    )
    try:
