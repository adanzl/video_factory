...
def design_size_for_source(source_width: int, source_height: int, settings: Settings) -> tuple[int, int]:
...
def resolve_auto_intro_size(*, settings: Settings, job: dict, media_dir: Path) -> tuple[int, int] | None:
...
def resolve_intro_size(*, settings: Settings, orientation: str | None, job: dict, media_dir: Path) -> tuple[int, int]:
    ...
    if is_material_job(job):
        ...
        if size:
            return size
        logger.warning('intro auto size fallback to portrait for material job %s (no base_meta/base.mp4/material dimensions)', job.get('id'))
    return portrait_size(settings)
