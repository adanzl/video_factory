    ...
    for i, row in enumerate(rows, 1):
        ...
        if theme_object:
            for j, row in enumerate(rows, 1):
                ...
                if (
                    sp == "妈妈"
                    and re.search(r"站好|过来|罚|今晚|别想|拿的什么", ln)
                    and theme_object not in ln
                ):
                    out.append({
                        "lines": [j],
                        "kind": "妈妈物证",
                        "desc": f"妈妈句未点名主题物：{ln}",
                        "fix": (
                            f"妈妈句须点名主题物「{theme_object}」"
                            "（可带碎渣/满地等主证据），勿只写笼统的满地狼藉；"
                            "短惩罚令与说话人不变"
                        ),
                    })
                    break
...
def build_review_prompts(theme: str, story: dict) -> tuple[str, str]:
    ...
    system = (
        ...
        "一会儿又被说成跑掉了；"
        "或物理常识反了：相框/纸/本子是扁平物不会「滚」，只有球/瓶盖才会滚）。\n"
        "2 错位：谁做什么前后对不上"
        "（例：约定「我藏车你藏零食」，下一句却说成「你塞零食我塞车」；"
        "或某人做的事后来被说成另一人做的；"
        "或连锁里把坏结果归到对方动作上——自己挤胶却怪「你手一抖」）。\n"
        "3 示范：大人开口指导孩子去隐瞒或说谎"
        ...
        "- B 类「结盟翻车」段5双人定格短句（完了/死定了/被发现了/这下完了…）"
        "是标准收束，不算空应答/占行/水词，别报。\n"
        "- I 类「问倒收束」：收束前赢家「还嘴硬？我不说了还不行？」"
        "是收摊试探，末段「哼，看你还嘴硬/一招制敌」是制敌收束；"
        "两句都带「嘴硬」但是递进（先赌气收摊→对方也闭嘴→再制敌），"
        "别当重复报；只有中段同一质问来回两圈以上才算重复。\n"
        "- J 类「权威压住」：昭昭反复求放行、灿灿反复「我说了算/不行」"
        "是哀求加码与否决压住，别当重复报；末段灿灿仍占上风、昭昭怂退"
        "是标准收场，勿按 A 式末四拍反噬理解。\n"
        "- K 类「家长看戏」：越劝越凶、互骂来回是升级链，别当重复报；"
        "末段僵持不和好、勿报缺破功；禁止 H 式拉手/不打了当收束。\n"
        "- L 类「退让点破」：成人催让+拒收+点破偏心后勿再开第二轮争夺，"
        "拒收句与点破句递进，别当重复报。\n"
        "- 妈妈短惩罚令（站好/过来/交出来/罚…）单独成句是权威落槌，"
        "不算空应答/占行，别报。\n"
    ...
    *,
    ...
    if full_scan or not issues:
        system += (
            ...
            "- 道具运动方式不符形状：扁平/片状物（相框/纸/本子）不会「滚」，"
            "只能「滑/掉/掀/飞」（「相框滚到柜子脚」→「相框滑到柜子脚」）；"
            "球/瓶盖等圆形物才会滚；\n"
            "- 措辞级逻辑矛盾：句子内部自相矛盾时直接改字修，"
            "如「我够不着，你手短，你去捡」→「我够不着，你手长，你去捡」"
            "（够不着应让手长/个子高的去够，别让更够不着的人去）；\n"
            "- 妈妈惩罚句笼统：只写「满地狼藉/一片狼藉/乱七八糟」时，"
            "要点名本场核心物（开场被弄坏/藏起/偷吃的那件东西），"
            "如「满地玻璃渣，相框还裂着，站好！」；短惩罚令与说话人不变；\n"
            "- 段4互甩只许 2 句（各1句），禁止新增甩锅/互怼对白；"
            "结尾定格照抄节拍表，段落后不再加对白；\n"
            "- 段3连锁至少 1 处儿童夸张感知（黏糊糊说成「手和本子黏在一起了」"
            ...
            "「我一伸手，碰倒了花瓶」），"
            "也抓多步因果解说（如「你绊倒了，相框又飞出去，砸到茶几腿」"
            "——一句念完两三步因果，谁绊谁拿都没交代），"
            "也抓「结果+拟声词连念」（如「花瓶倒了，哗啦！」「瓶子掉了，啪嗒！」"
            "——把结果和声音一起念完是解说；改成进行时惊呼「花瓶！要倒——！」"
            "「瓶子！掉了——！」，拟声词留给下一拍），"
            "要改成当场惊呼或正在进行的动作（「哎呀胶水！滑——」「相框！又掉了！」），"
            "结果留给画面和下一个反应；\n"
