...
- 若要改口，须有新理由（被字面戳穿、被证据打脸、对方换条件），不能为收束硬拧。
...
4. 禁止为凑长度反复换说法车轱辘。
5. 禁止后半段换冲突、换地点主场、新开一件事。
6. 禁止角色无铺垫的自相矛盾（立场/证据前后打架）。

【笑点与收束】
- 笑点 = 孩子的字面/现实逻辑 碰撞 姐姐的「装大人」规则，或两人各执一词越辩越歪。
- 每一句推论须基于刚听到的字面意思或亲眼见过的生活经验，不能跳级。
- 在 300–380 字内把误会滚大再收束；末句宜落在破功/哑口/顺势软收，但须承接上一拍，禁止硬拧收场。
- 软收可以（认栽、哼一声放过），前提是前面已被戳穿或理由说得通。
...
8. 立场须连贯：可软收，禁止无铺垫自相矛盾（如刚喊不公平立刻好吧）。
...
5. 【开场首镜】scene_id=1 须定格冲突峰值（抢/举/夺/藏），shot_type 优先「特写」或「中景」；
   禁止用全景空镜或寒暄站桩开场。
