...
def expansion_outline(bp: dict) -> str:
    ...
    return (
        ...
        f"5. 昭昭回旋镖（倒数第 2 句）：**从以下 5 个模板中选 1 个，填空即可，禁止自由发挥**。\n"
        ...
        "   - 【抄叮嘱原话】= 一字不改抄 key_line"
        f"「{bp.get('key_line') or ''}」\n"
        "   - 【灿灿刚做的破规动作】= 第4步灿灿亲手做的动作，"
        "**用她上一句对白里的原词**（她喊了「揉掉」就写揉掉、"
        "喊了「哗啦倒完」就写哗啦倒完、喊了「扫进」就写扫进）\n"
        "   - 把模板里的【】替换成实际内容，其余字（你自己说/怎么现在）一个不许改\n"
        f"   - boom 参考：{bp.get('boom') or ''}（只是方向提示，实际台词必须用上面5个模板之一）\n"
