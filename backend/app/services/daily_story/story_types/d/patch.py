...
加规则红线（新增前必读）：
- 只允许四类操作：**删句/删词、句内去重、改 speaker、引话接地**
  （从前文灿灿原话抽子串对齐引用）。
- 禁止任何「整句模板改写 / 按主题关键词造台词」——
  2026-07 浇花案例：兜底模板把中段整段盖成空话还刷高观感分。
  内容不合格一律交 LLM 重试，不在本地造句。
"""
...
def patch_d_trim_waffle(story: dict) -> list[str]:
    ...
    for i in range(2, len(dialogue) - 4):
        ...
        if _WAFFLE.search(line):
            ...
            return notes + patch_d_trim_waffle(story)
    return notes


def patch_d_closing_speaker(story: dict) -> list[str]:
    """末句若为妈妈但无破功语气，且上一句已是回旋镖，改末句 speaker。"""
    notes: list[str] = []
    if not _is_d(story):
        return notes
    dialogue = story.get("dialogue")
