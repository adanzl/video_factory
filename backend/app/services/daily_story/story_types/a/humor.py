...
def humor_revision_hint(issue: str) -> str | None:
    ...
    if "末四拍不完整" in issue:
        return (
            "【A·末四拍】末尾连续4句须完成：昭昭引用灿灿前文原话→"
            "灿灿给当前动作找具体例外→昭昭追问例外为何成立→灿灿软破功；"
            "只锁语义功能，不强制固定句型；只改末4句。"
        )
    ...
    if "空甩身份" in issue:
        return (
            "【A·收束借口】倒数第3句须针对本场动作给具体例外，"
            "勿空甩「我是姐姐」，勿借用其他故事的道具或动作。"
        )
    if "未扣一锤" in issue:
        return (
    ...
    if "好笑不足" in issue or "格式达标但好笑" in issue:
        return (
            ...
            "翻车落到可数细节（第几口/几下/哪个数）；"
            "末四拍按引用→具体例外→追问例外→让步收束，措辞按本场自然生成。"
        )
    return None
...
def _close_four_beat_complete(
    ...
    if len(tail4) < 4:
        return False
    words_ok = bool(
        _RE_DIRECT_QUOTE.search(tail4[-4])
        and re.search(
            r"不算|只是|这次|刚才|例外|因为|我这是|教你|示范|检查|试试|本来",
            tail4[-3],
        )
        and re.search(r"？|吗|呢|凭什么|怎么|为何|哪", tail4[-2])
        and re.search(r"哼|行吧|随便|好吧|算了|给你|不管了|认栽", tail4[-1])
    )
    if not words_ok:
        return False
