...
L_OPENING_ANCHOR_RE = re.compile(
    r"争|抢|该我|也要|公平|给他|给她|让给|这瓶|这盘|碗里"
)
L_OPENING_BAD_RE = re.compile(r"爱学习|你爱吗|灵魂|拷问|八百|吃商")


def append_l_opening_errors(
    ...
    *,
    ...
    if (type_code or "").upper() != "L":
        return
    if not normalized:
        return
    first = str(normalized[0].get("line") or "").strip()
    blob = f"{setting}{conflict_core}{first}"
    if L_OPENING_BAD_RE.search(first):
        errors.append("L类开场：首句勿灵魂拷问/吃商收束信号")
    if not L_OPENING_ANCHOR_RE.search(blob):
        errors.append("L类开场：宜点争物或公平催让信号")
...
def score_opening_quality(story: dict) -> tuple[int, list[str], list[str]]:
    ...
    if L_OPENING_BAD_RE.search(first):
        cons.append("L开场：首句勿拷问/吃商信号")
        pts -= 4

    blob = f"{setting}{conflict_core}{''.join(lines_o)}"
    if L_OPENING_ANCHOR_RE.search(blob):
        pts += 2
        pros.append("L开场锚定争物/催让")
    else:
        cons.append("L开场缺争物/催让信号")
        pts -= 2
