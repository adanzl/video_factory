...
LINE_K = StoryTypeLine(
    ...
    prompt_block="""\
【本次类型：K 家长看戏 — 专属线路】
- 收场看 **k_close_mode**（生成前锁定）：
  · K_A_PARENT_FAIL_STALEMATE：互打升级→大人劝失败→僵持不和好；
  · K_B_CHILD_SELF_RESOLVE：互打宜短→挡回/不评理旁观→孩子自行恢复互动；
  · K_UNKNOWN：只跟 beat_chain，勿擅自补劝失败或僵持。
- 共同：禁止 H 式定责仪式和好。
- **不是** H 第三方化解——大人不定责劝成、不拉手和好。
- **不是** A/J——主戏是互骂互打，不是权威一锤或否决压住。
- **不是** F 互呛加码——K 笑点在大人**劝不动/看戏**，不是外部打断熄火。

【三段链（M12+K · 看 k_close_mode）】
- **K_A**：互骂升级→大人劝失败→僵持不和好。
- **K_B**：互打宜短→挡回/不评理/吃饭旁观→孩子自行恢复互动→
  家长**自言自语**点题（不掺和）；closing 可对爸爸说，对白勿喊爸爸、勿解说「你看他俩」。
- **K_UNKNOWN**：只跟 beat，勿擅自补劝失败或僵持。

【硬约束】
- 妈妈/爸爸台词 1–3 句；K_A 宜劝失败，K_B 宜挡回旁观。
- **禁止** H 式拉手/齐声不打了/道歉和好。
- punchline 勿写正文未呈现动作；对白禁止「勾肩搭背」等分镜旁白，用一起走/一起吃说。
""",
    user_closing="""\
