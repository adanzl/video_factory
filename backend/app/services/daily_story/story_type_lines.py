...
- **可核对事实**：一旦写出钟点、时长、算式、遍数等，全剧只认一套数，勿前后改起点或改口；
  没写具体数字则不必硬编钟点。
- **刷牙/计时**：规则时长、弟弟刷了多久、姐姐自己刷了多久——各至多一个数；
  禁止开场「半分钟」正文又「一分半」；禁止「才一分钟」又说「正好两分钟」。
- 中段抬杠须围着这一锤升级；禁止通篇只辩论「我是姐姐」「你得听我的」而无新事实。
- 同一旧账（折纸、上次查资料）**最多提 1 次**；再要加码须换证据或回到本场一锤。
- 收束「你刚才说…」必须引用**灿灿前文原话**；禁止昭昭先自造一句再假装引用。
...
【A 类开场补充】
- 时间：在灿灿开口管教**之前**的一瞬（如姐姐正刷手机笑、弟弟伸手够不到）。
- 可写灿灿定场（「作业写完才能碰手机」）或昭昭只描述看见的画面；**勿写**对接正文第 1 句的「还说我磨蹭/还不许」。
- **禁止开场揭穿一锤**：勿写「你自己才刷半分钟/草稿算错/刚玩过」——翻车留给正文中段。
- 反例：正文将写「别磨蹭了」→ 开场忌「还说我磨蹭」（指责还没出口）。
- 反例：「谁先洗澡」「酸奶你怎么开了」（那是 C，不是 A 开场）。
- 刷牙反例：「姐你计时器上自己才刷了半分钟」（剧透翻车，不合格）。
""",
    opening_user_append="""\
本场为 A 权威翻车：开场早于正文首句，只定格管教前现场；勿用「还」接正文里才要说的管教词；勿开场揭穿灿灿已翻车。
""",
    theme_user_append="""\
...
""",
    retry_soft_close_hint=(
        ...
        "【A·事实】写了钟点、时长或算式须全剧一致；刷牙/计时各角色时长只留一个数；"
        "约定X分钟且才Y分钟时勿说时间到了。"
        "【A·埋句】补灿灿一句≤14字可引用原话；收束禁止引用灿灿没说过的话。"
