...
【正例骨架（刷牙·好笑）】
- **字数靠「立规前抬杠」+「一锤后追问」凑满280**；
  「吐水也算停」到「噗」之间最多再1来回，禁止用三十下/认真数注水凑字。
- 开场（发现现场）：牙膏沫/才刷几下。
- 抬杠2–3来回：凭什么管、两分钟怎么算、你又不是老师。
- 立规+埋句：「连续刷，吐水也算停」。
- 昭昭：你示范一下。→ 灿灿：一、二、三——噗。
- 追问1–2来回：才刷几下也叫连续？你刚说吐水算停。
- 末四拍：引「吐水也算停」→那不一样（示范/吐泡沫等具体借口）
  →哪里不一样→哼。
- **禁止**：示范前三十下/认真数/帮你盯；收束禁止只回「我是姐姐」。

【末四拍收束（硬约束，顺序不可跳）】
- 倒数第4：昭昭「你刚才说/你自己说 + **前文灿灿原话**」（须扣一锤，勿引空规矩）
- 倒数第3：灿灿「那不一样……」（狡辩区分）
- 倒数第2：昭昭「哪里不一样？都是听/都是……」（**全文仅此一处**）
- 末句：灿灿「……哼/行吧/随便」（必须灿灿说；**句内勿再发新指令**管昭昭）
...
15. punchline_explain 以「A类权威翻车」开头，写明一锤是什么、引用了哪句原话。
""",
    body_user_anchor="""\
...
""",
    retry_soft_close_hint=(
        "【A·好玩】须有一、二、三——噗或才两三下就吐；"
        "吐水算停后立刻示范翻车；字数用立规前抬杠+一锤后追问补，"
        "删三十下/认真数/帮你盯。"
        "【A·收束】那不一样须具体借口（示范/吐泡沫），禁止只说我是姐姐。"
        "【A·扣一锤】只引灿灿真说过的吐水算停；末句只哼/行吧/随便。"
        "【A·去注水】删换位置/数着/默数/电子表。"
    ),
    escalation_revision_hint=(
        "【A·好玩】压缩到：立规→示范→数两下就噗→末四拍；删预热。"
        "【A·埋句】灿灿先说吐水也算停；收束勿只甩我是姐姐。"
    ),
    layer_patterns=_compile_layers(
        [
            (
