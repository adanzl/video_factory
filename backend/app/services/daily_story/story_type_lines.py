...
- 正文 L2–L3 须出现**一场面级细节**，与 theme 同一件事，观众能想象画面：
  算式与得数（如 36+57 算出 93）、弹错哪个音、多玩几分钟、哪道题写错等。
...
【正例骨架（教作业）】
- 一锤：灿灿验算 36+57=93，还要昭昭改 92。
- 埋句：灿灿「我错一次就不算数」。
- 收束：昭昭「你刚才说错一次就不算数」→「那不一样我是教你」
  →「哪里不一样？都是听」→灿灿「……哼，你自己再算算」。
""",
    user_closing="""\
9. 【A类正文锚定】首 2 句管教现场；首句勿抢物、争先后。
10. 【A类·本场一锤】L2–L3 必须写出可画细节（数字/音名/分钟数/具体题），
    conflict_core 与一锤同一件事；无数字或具体动作则视为未达标。
...
""",
    retry_soft_close_hint=(
        "【A·好笑】若无「本场一锤」（算错数/弹错音/超时等），在中段补一场面细节。"
        "【A·埋句】补灿灿一句≤14字可引用原话。"
