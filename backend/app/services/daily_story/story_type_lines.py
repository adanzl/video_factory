...
【正例骨架（刷牙·好笑）】
- 立规：灿灿「必须连续刷两分钟，停手就重来」。
- 埋句：灿灿「吐水也算停」。
- 一锤：示范时刷两三下就吐水（或边刷边看手机）。
- 收束：昭昭引「吐水也算停」→「那不一样」→「哪里不一样？都是停」→灿灿「……哼」。
- 末句只许嘴硬认栽，禁止哼完还继续管昭昭「你先重刷」。
...
""",
    retry_soft_close_hint=(
        "【A·好笑】一锤须可拍荒谬；刷牙：立「吐水算停」→示范→刷几下就吐→末四拍，中间勿吵换位置。"
        "【A·去注水】删换位置/数着/别数/默数/电子表；立完规矩立刻翻车。"
        "【A·扣一锤】收束只引灿灿真说过的话（吐水算停），勿自造「停手就重来」。"
        "【A·次数】灿灿刷了几下全文只留一个数，禁止先说很多下后又才刷两下。"
        "【A·末句】只许哼/行吧/随便，禁止「算你厉害」。"
        "【A·末四拍】昭昭引原话→灿灿那不一样→昭昭哪里不一样→灿灿哼。"
    ),
    escalation_revision_hint=(
        "【A·一锤】当场刷几下就吐水；删换位置争论。"
        "【A·埋句】灿灿先说「吐水也算停」，再翻车，再被引用。"
        "【A·去复读】中段同一规矩勿讲两遍。"
    ),
    layer_patterns=_compile_layers(
        [
