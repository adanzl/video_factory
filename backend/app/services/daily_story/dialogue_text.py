...
OPENING_PLACE_RE = re.compile(
    ...
    r"卫生间|浴室|洗手台|水槽|阳台|地板|抽屉|书包|餐桌|冰箱|挂钟|茶几",
)
OPENING_VISUAL_RE = re.compile(
    r"亮着|歪|松了|鼓鼓|攥|抢|藏|溅|沫|泡泡|壳|屏幕|勺子|嘴角|"
    r"泡沫|散了|拖|空着|少了|黏|烫|倒|洒|堆|指向|红红|"
    r"一堆|塞|歪着|系一块|打开了|还亮|"
    r"一包|一袋|一块|零食|饼干|薯片|巧克力|糖果|"
    r"发现|滚出|露出|藏着|看到|底下|下面|茶几|"
    r"碎片|碎渣|渣|杯子|水杯|"
    # 指示词 = 指点动作（2026-08-10 用户）：台词里有「这」字，孩子多半手指点着眼前
    # 实物在说（这块/这个/这盒/这排），本身就是可拍画面——「书桌上这块新橡皮」说这句
    # 时是指着橡皮的。
    r"这",
)
