...
from app.core.pipelines import PIPELINE_MATERIAL, is_material_job, resolve_pipeline, stage_index, stages_for
...
def _material_script_reset_seed(job: dict) -> dict | None:
    ...
    if pending:
        return {'pending_narration': pending, 'script_mode': script_json.get('script_mode', 'manual')}
    if script_json.get('script_mode') == 'manual' and script_json.get('narration'):
        return {'pending_narration': script_json['narration'], 'script_mode': 'manual'}
    return None
...
def _archive_wan_clips(media_dir: Path, *, clip_names: list[str] | None=None) -> None:
    ...
    if not clips_dir.exists():
        return
    clips = [clips_dir / name for name in clip_names] if clip_names else sorted(clips_dir.glob('*.mp4'))
...
def _archive_wan_merge(media_dir: Path) -> None:
    ...
    for src_name, dest_name in (('final.mp4', 'final_wan.mp4'), ('body.mp4', 'body_wan.mp4'), ('body_with_audio.mp4', 'body_with_audio_wan.mp4')):
...
def _fs_clear_merge_artifacts(media_dir: Path) -> None:
    ...
    _delete_files([media_dir / 'final.mp4', media_dir / 'body.mp4', media_dir / 'body_with_audio.mp4'])

def _db_clear_tts(job_id: int) -> None:
    repo_segment.clear_segment_durations(job_id)
    repo_segment.clear_segment_clips(job_id)
    repo_job.update_job(job_id, audio_path=None, subtitle_path=None, tts_usage_json=None)
...
def _clear_tts_artifacts(job_id: int, media_dir: Path) -> None:
    ...
    with atomic():
        _db_clear_tts(job_id)
    _fs_clear_tts(media_dir)

def _db_clear_segment_clips(job_id: int) -> None:
    repo_segment.clear_segment_clips(job_id, None)
...
def _db_clear_partial_segment_media(job_id: int, segment_indices: list[int]) -> None:
    repo_segment.clear_segment_clips_only(job_id, segment_indices)

def _fs_clear_partial_segment_media(media_dir: Path, segment_indices: list[int]) -> None:
...
def _db_clear_segment_images(job_id: int, segment_indices: list[int] | None) -> None:
    repo_segment.clear_segment_media(job_id, segment_indices)

def _fs_clear_segment_images(media_dir: Path, segment_indices: list[int] | None) -> None:
    if segment_indices:
        for index in segment_indices:
...
def _db_clear_downstream(job_id: int, stage: str, job: dict) -> None:
...
def _db_clear_stage_self(job_id: int, stage: str, job: dict, *, segment_indices: list[int] | None, segment_scope: str | None=None) -> None:
    """清空 stage 自身 DB 产物，以便重新执行。"""
    pipe = resolve_pipeline(job)
    if stage == 'prepare':
        repo_job.update_job(job_id, base_path=None)
        return
    if stage in {'title', 'script'}:
        ...
        repo_job.update_job(job_id, script_json=reset_script_json, intro_path=None, cover_path=None, end_path=None, final_path=None, quality_report=None)
        return
    if stage == 'intro':
        repo_job.update_job(job_id, intro_path=None, cover_path=None, end_path=None)
        return
    if stage == 'tts':
        _db_clear_tts(job_id)
        return
...
def _fs_clear_stage_self(stage: str, media_dir: Path, job: dict, *, segment_indices: list[int] | None, segment_scope: str | None=None) -> None:
    ...
    if stage == 'intro':
        if media_dir.exists():
            _delete_files([media_dir / 'intro.mp4', media_dir / 'intro.png', media_dir / 'cover.jpg', media_dir / 'end.mp4'])
        return
    if stage == 'tts':
        _fs_clear_tts(media_dir)
        return
...
def prepare_rerun(job_id: int, action: str, *, segment_indices: list[int] | None=None, mode: str='from') -> dict:
    ...
    if segment_indices is not None:
        if stage != 'segment':
            raise ValueError('segments 仅可用于 segment / segment/all / segment/images / segment/clips')
    ...
    with atomic():
        ...
        if is_material_job(job) and (action.startswith('segment/') or action == 'segment'):
        ...
        repo_job_log.append_log(job_id, stage, detail)
    _fs_clear_stage_self(stage, media_dir, job, segment_indices=segment_indices, segment_scope=segment_scope)
    _fs_clear_downstream(stage, media_dir, job)
    return job

def prepare_for_action(job_id: int, action: str, *, segment_indices: list[int] | None=None) -> dict:
    """兼容旧名 → :func:`prepare_rerun`。"""
    return prepare_rerun(job_id, action, segment_indices=segment_indices, mode='from')

def prepare_job_rerun(job_id: int, stage: str, *, segment_indices: list[int] | None=None, mode: str='from') -> dict:
    """兼容 CLI 旧名 → :func:`prepare_rerun`（``stage`` 当作 action）。"""
    return prepare_rerun(job_id, stage, segment_indices=segment_indices, mode=mode)

def reset_job_from_stage(job_id: int, from_stage: str, *, segment_indices: list[int] | None=None) -> dict:
    return prepare_rerun(job_id, from_stage, segment_indices=segment_indices, mode='from')
