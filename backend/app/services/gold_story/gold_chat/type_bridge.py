...
_M12_K_B_CHAIN: tuple[str, ...] = (
    ...
    "家长末句自言自语点题（不掺和/不评理；勿对观众解说、勿对白喊爸爸）",
)
...
_MECH_HINT_APPEND: dict[tuple[str, ...], str] = {
    ...
    ("M12", "K", "K_B_CHILD_SELF_RESOLVE"): (
        ...
        "禁止 H 式定责道歉拉手；"
        "closing_intent 若写「对爸爸说」，成稿仍用妈妈**自言自语/嘀咕**点题，"
        "勿写「跟孩他爸说/咱俩」、勿「你看他俩自己就好了」式解说；"
        "挡回时宜带吃饭/端碗旁观（勿只写照料指令）；**禁止**家长说「接着打/继续打/你俩打」"
        "等怂恿冲突；互打段宜短（争锋+挡回后≤6句孩子互顶），忌重复争「谁先」；"
        "忌堆「不服/试试看/还没完」无新信息叫阵；句尾禁「…了呢…」等破碎垫字；"
        "同一句内重复口头词（如连用两个「试试看」）交 gold_chat **润色**定点改，"
        "不靠 K 类型 patch 硬替；"
        "「谁怕谁/不让步」全篇宜≤2处；自行恢复只保留一轮邀约+接住，勿叠两轮；"
        "punchline_explain 只写正文已呈现收束，勿写勾肩搭背等未写出动作；"
        "孩子自行恢复须**口语邀约**（一起吃/一起走/还玩不玩），"
        "禁止「勾肩搭背」等分镜姿态词写入对白；"
        "禁止末段堆「不理你/谁稀罕」冷战当收束"
    ),
    ("M12", "K", "K_UNKNOWN"): (
...
def structure_type_hint(
    *,
    ...
    if st == "K" and k_mode == "K_B_CHILD_SELF_RESOLVE":
        ...
        parts.append(
            "- 收束：孩子邀约/一起走；家长末句自言自语点题；禁僵持劝失败模板"
        )
