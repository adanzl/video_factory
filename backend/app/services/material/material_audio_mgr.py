...
class MaterialAudioMgr:
    ...
    def list_materials(self, *, limit: int=50, offset: int=0) -> dict:
        """返回 {items: [...], total: N}。"""
        with atomic():
            items = repo_material_audio.list_material_audios(limit=limit, offset=offset)
            total = repo_material_audio.count_material_audios()
            return {'items': items, 'total': total}

    def get_material(self, material_id: int) -> dict:
        return repo_material_audio.get_material_audio(material_id)
    ...
    def update_material(self, material_id: int, **fields: object) -> dict:
        ...
        return repo_material_audio.update_material_audio(material_id, **updates)

    def delete_material(self, material_id: int) -> None:
        settings = get_settings()
        with atomic():
            repo_material_audio.get_material_audio(material_id)
            repo_material_audio.soft_delete_material_audio(material_id)
    ...
    def _finalize_audio_file(self, material_dir: Path, dest: Path) -> dict[str, object]:
        ...
        return {'file_path': str(dest), 'duration_sec': duration if isinstance(duration, (int, float)) else None, 'size_bytes': dest.stat().st_size}
    ...
    def upload_material(self, file: FileStorage, *, name: str | None=None, note: str | None=None) -> dict:
        ...
        try:
            with atomic():
                material = repo_material_audio.create_material_audio(name=display_name, file_path=_PLACEHOLDER_PATH, note=note_text)
    ...
    def edit_material(self, material_id: int, *, name: str, note: str | None=None, file: FileStorage | None=None) -> dict:
        ...
        if file and file.filename:
            ...
            dest = self._write_audio_file(material_dir, file, ext)
            meta = self._finalize_audio_file(material_dir, dest)
        return repo_material_audio.update_material_audio(material_id, name=cleaned_name, note=note_text, **meta)
