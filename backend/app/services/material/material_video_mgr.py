...
class MaterialVideoMgr:
    ...
    def list_materials(self, *, limit: int=50, offset: int=0) -> dict:
        """返回 {items: [...], total: N}。"""
        with atomic():
            items = repo_material_video.list_material_videos(limit=limit, offset=offset)
            total = repo_material_video.count_material_videos()
            return {'items': items, 'total': total}

    def get_material(self, material_id: int) -> dict:
        return repo_material_video.get_material_video(material_id)
    ...
    def update_material(self, material_id: int, **fields: object) -> dict:
        ...
        return repo_material_video.update_material_video(material_id, **updates)

    def delete_material(self, material_id: int) -> None:
        settings = get_settings()
        with atomic():
            repo_material_video.get_material_video(material_id)
            repo_material_video.soft_delete_material_video(material_id)
    ...
    def _finalize_material_video(self, material_dir: Path, dest: Path) -> dict[str, object]:
        ...
        return {'file_path': str(dest), 'duration_sec': duration, 'width': width, 'height': height, 'size_bytes': dest.stat().st_size, 'thumbnail_path': str(thumb)}

    def upload_material(self, file: FileStorage, *, name: str | None=None, note: str | None=None) -> dict:
        ...
        try:
            with atomic():
                material = repo_material_video.create_material_video(name=display_name, file_path=_PLACEHOLDER_PATH, note=note_text)
    ...
    def edit_material(self, material_id: int, *, name: str, note: str | None=None, file: FileStorage | None=None) -> dict:
        """编辑素材：名称、备注；可选更换视频文件。"""
        ...
        if file and file.filename:
            ...
            dest = self._write_material_video(material_dir, file, ext)
            meta = self._finalize_material_video(material_dir, dest)
        return repo_material_video.update_material_video(material_id, name=cleaned_name, note=note_text, **meta)

    def analyze_material(self, material_id: int) -> dict:
        """异步分析素材视频，立即返回。后台线程完成后写入 note。"""
        with atomic():
            material = repo_material_video.get_material_video(material_id)
            repo_material_video.update_material_video(material_id, status='analyzing')
        video_path = Path(material['file_path'])
        duration = material.get('duration_sec')
        t = threading.Thread(target=self._run_analysis, args=(material_id, video_path, duration), daemon=True)
        t.start()
        logger.info('material %s analysis thread started', material_id)
        return {'material_id': material_id, 'analyze_status': 'analyzing'}

    def _run_analysis(self, material_id: int, video_path: Path, duration: float | None) -> None:
        """后台线程：执行分析并写 DB。"""
        try:
            analyzer = VideoAnalyzer(video_path, duration=duration)
            note_text = analyzer.analyze()
            with atomic():
                repo_material_video.update_material_video(material_id, note=note_text, status='active')
            logger.info('material %s analysis complete', material_id)
        except Exception:
            logger.exception('material %s analysis failed', material_id)
            with atomic():
                repo_material_video.update_material_video(material_id, status='analyze_failed')

    def create_job_from_material(self, material_id: int, title: str, *, narration: str | None=None, script_mode: str='ai', skip_publish: bool=True, run_mode: str='prepare') -> dict:
        cleaned_title = re.sub('\\s+', '', title.strip())
        ...
        if mode == 'manual':
            ...
            if len(re.sub('\\s+', '', narration)) < NARRATION_ABS_MIN_CHARS:
                raise ValueError(f'narration too short (need >= {NARRATION_ABS_MIN_CHARS} )')
