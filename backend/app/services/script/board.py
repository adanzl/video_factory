...
_IMAGE_PROMPT_MOTION_TAIL = (
    ...
    "2) 若主体为人物或动物，禁止写肢体动作，但须写情绪或神态变化（如神情从疑惑转为恍然、眼神中透出警觉）；"
    "3) 可补充 1 个环境元素的联动变化（如主体运动带动的烟、水、光影反应）；"
    "4) 动态须有方向、速度、幅度等可感知细节，不要只写「轻微晃动」这类模糊词。"
    "镜头仅可极缓推近/拉远/平移，幅度很小。"
    "禁止：描述人物或动物的肢体动作（手指弯曲、眨眼、转身、行走等）；"
    "禁止：使用抽象特效词（光效、光晕、粒子、能量、光圈、脉动等），必须写真实可见的物理动态；"
    "禁止：套话「镜头固定，主体稳定，画面平滑」及同义填空。"
    "正例：丹炉炉盖被蒸汽顶起又落下，缝隙中白烟成股涌出向右飘散。"
    "正例：烧杯中蓝色液体从底部开始冒泡，气泡逐渐密集并带动液面缓慢上涌。"
    "正例：古书书页被风从右向左逐页翻起，书角翘起后缓缓回落。"
    "正例：沙盘上细沙从高处沿沟壑缓缓滑落，堆积的扇面逐渐扩大。"
    "正例：老者眉头微蹙，目光缓缓移向窗外，神情从沉思转为凝重。"
    "反例：小偷手指微微弯曲。（描述人物肢体动作）"
    "反例：镜头固定，主体稳定，画面平滑。（套话填空）"
