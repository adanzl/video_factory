...
def build_chat_title_system_prompt(*, max_title_len: int = CHAT_TITLE_MAX_LEN) -> str:
    ...
    return (
        ...
        "\n【标题句式模板】≤10字内选最合适一种，从剧本找最强钩子填入："
        "\n①台词钩子：冲突高潮的孩子原话，稍压虚词。例「老鼠会开柜子门吗」"
        "\n②反差设问：大人双标/口嗨被戳穿的问句。例「自己不吃却管我？」"
        "\n③意外后果：动作惹出没料到的结果。例「越擦越花」"
        "\n④悬念藏匿：藏/偷/瞒+藏起来的东西。例「妈妈藏的饼干」"
        "\n【标题生成规则】"
...
def build_chat_title_user_prompt(
    *,
    ...
    return (
        f"初稿标题：{draft_title}\n"
        f"剧本内容：\n{context}\n\n"
        f"请输出 ≤{max_title_len} 字的 title。先想清最强钩子，再压缩："
        "超字只删虚词/语气词，禁止删成事件名。"
    )
