...
_IMAGE_PROMPT_DIMENSIONS_FULL = (
    ...
    "主体（角色须写年龄/发型/脸型/服装/身高体型等外貌特征，与 visual_style 主角描述一致；姿态、动作）；"
    "③场景（前景/中景/背景）；"
...
_IMAGE_PROMPT_RULE_DAILY_STORY = (
    ...
    "「[改变要求] + [新风格/场景/动作] + [需添加/移除的元素] + [需保留的元素]」。"
    "不得仅写角色名字，必须写明保留参考图中该角色的外貌特征（发型、脸型、服装等）。"
    "例子：'基于参考图角色形象，将场景改为客厅争执，添加昭昭抱拼图盒子、灿灿叉腰动作和涂鸦风格，同时保留参考图中昭昭的黑色短发圆脸蓝色短袖T恤和灿灿的马尾辫粉色卫衣。'"
)
