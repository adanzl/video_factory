...
_DAILY_VISUAL_BRIEF_CONTENT_RULE = (
    ...
    "【地点与陈设】每镜开头必须写清地点（与全片 setting 一致），"
    "即使写接上一镜也要先写地点；"
    "背景陈设沿用 setting 已建立的固定陈设，只首镜写全，"
    "后续写「场景同前/背景同分镜1」，不重复罗列；"
    "仅当本段台词点名其他背景物件时再改写陈设；"
    "【承托物/容器】若台词或剧情涉及托盘/盘子/水盆等承托物，"
    "首镜起就必须写明（如「花盆下方有托盘」），"
    "后续镜只写状态变化（干→湿→积水→满），"
    "禁止前几镜完全没有、后几镜突然出现；"
    "浇花/花盆场景中花盆里必须有一株盛开的红花，花朵明显可见，枝叶完整；"
    "【道具位置固定】核心道具（花盆/托盘/水壶等）的摆放位置全片固定"
    "（如花盆放在地面/花架上），每镜地点句写明同一位置；"
    "禁止同一道具在不同镜出现在桌子/地面/窗台等不同位置。"
    "【冲突道具状态】本片 setting 与台词点名的核心冲突道具必须每镜写清当前状态，"
    "状态随剧情单向推进，禁止所有分镜写成同一状态；"
    "台词出现状态变化词时，该镜及之后状态必须已变化；"
    "状态句放在地点句之后，使用本片自己的物件名，不套用其他故事的物件。"
    "【剧情连续性】先按全片 narration 明确本镜剧情拍"
    "（本镜推进哪一拍），再写画面；"
    "相邻镜须因果承接：上一镜的持物/手势/道具状态本镜要延续或明确变化，"
    "禁止道具凭空出现/消失；全片固定人物站位；"
    "每镜开头可用「接上一镜/此时」承接，避免每镜都像独立截图；"
    "两人始终同框清晰入画，禁止把其中一人写成背景模糊/远景；"
    "禁止括号内心理/解释说明，只写可见画面。"
    "【安全】儿童角色不得持真实刀具/锐器/危险品；"
    "若剧情涉及刀具，写「塑料道具刀/玩具刀」并写明非锋利，"
    "禁止「水果刀」「餐刀」「锋利刀具」等表述。"
    ...
    "禁止同一角色同时出现两组手部动作（如双手做一组动作又另有手拿道具）；"
    ...
    "若首说话人实际不开口（沉默收尾/哼/翻白眼），brief 必须写「闭嘴/不说话/嘴角下撇」，"
    "不要写正在说话；"
    "其他角色禁止写说话/反驳/大喊/开口等，只用眉眼肢体表达情绪；"
    "【硬性】非首说话人出现「说话/反驳/大喊/开口/嘴巴张开」任一词汇即整段不合格，必须重写。"
    "【持物一致性】同一道具的持物手全片统一（默认右手持物），禁止左右手跳变；"
    "持物手与相邻镜保持一致；"
    "持物角色在持物期间不得写双手摊开/叉腰/抱臂等无物动作，"
    "持物手必须保持持物状态，另一只手可做其他动作；"
    "道具交接后原持有者不再接触道具，交接帧写明「已松手/仅指尖轻触」；"
    "道具易主必须写明「已拿到/已夺过/已交接」，禁止没有交接直接跳到新持有者；"
    "易主镜禁止同时写旧持有者仍持物与新持有者已拿到；"
    "夺取镜写成「水壶已到XX手里，旧持有者双手已松开」；"
    "交接完成帧只写新持有者已握住，原持有者双手已松开/垂在身侧，"
    "禁止再写原持有者递出/拿着；"
    "交接以台词为准：交接完成只发生在台词明确交接的分镜；"
    "上一分镜只写递出/伸手，不写对方已握住；"
    "后续镜由同一角色持续持有，直至再次交接；"
    "背景陈设不出现与当前道具重复的第二件同款道具；"
    "持物角色非持物手在同一阶段保持同一基准姿态（自然下垂/轻扶），"
    "不要每镜换叉腰/背手/摊开，仅情绪重音镜可变化一次。"
    "【道具】冲突道具用台词已出现的物件与状态；"
...
def normalize_daily_visual_brief_sequence(segments: list[dict]) -> list[dict]:
    ...
    has_tray = any("托盘" in str(seg.get("visual_brief") or "") for seg in segments)
    has_pot = any("花盆" in str(seg.get("visual_brief") or "") for seg in segments)
    for seg in segments:
        ...
        # 交接完成帧：不写原持有者递出/接过，统一成「已交给/双手握住」
        vb = re.sub(r"右手将([^，。；]{1,8})递给", r"已将\1交给", vb)
        vb = re.sub(r"伸出双手接过([^，。；]{1,8})", r"双手握住\1", vb)
        vb = re.sub(r"双手接过([^，。；]{1,8})", r"双手握住\1", vb)
        m_transfer = re.search(
            r"(昭昭|灿灿|妈妈)已将([^，。；]{1,8})交给(昭昭|灿灿|妈妈)",
            vb,
        )
        if m_transfer:
            old, prop, holder = m_transfer.group(1), m_transfer.group(2), m_transfer.group(3)
            vb = re.sub(
                rf"{re.escape(old)}已将{re.escape(prop)}交给{re.escape(holder)}[，,][^。；]*",
                f"{prop}已在{holder}手中，{old}双手背在身后",
                vb,
            )
        vb = vb.replace("接过", "握住")
        vb = vb.replace("递给", "交给")
        ...
        vb = re.sub(r"[，,]\s*神情(?=[。；]|$)", "", vb)
        vb = vb.replace("托盘仍在原位置", "托盘在花盆正下方")
        vb = vb.replace("花盆放在花架上", "花盆放在地面")
        vb = vb.replace("花盆放在花架", "花盆放在地面")
        vb = vb.replace("放在花架上", "放在地面")
        vb = vb.replace("矿泉水瓶", "浇花水壶").replace("饮料瓶", "浇花水壶")
        vb = re.sub(r"(?<!浇花)水壶", "浇花水壶", vb)
        vb = vb.replace(
            "花盆里有一株花，枝叶完整。",
            "花盆里有一株盛开的红花，花朵明显可见，枝叶完整。",
        )
        vb = vb.replace(
            "花盆在画面中，花盆里有一株花，枝叶完整。",
            "花盆在画面中，花盆里有一株盛开的红花，花朵明显可见，枝叶完整。",
        )
        if "花盆" in vb and "一株" not in vb:
            vb += "花盆里有一株盛开的红花，花朵明显可见，枝叶完整。"
        if has_pot and "花盆" not in vb:
            vb = (
                "花盆在画面中，花盆里有一株盛开的红花，花朵明显可见，枝叶完整。"
                + vb
            )
        if "尚未交接" in vb or "悬在中间" in vb:
            holder = "昭昭" if "昭昭" in vb else ("灿灿" if "灿灿" in vb else "对方")
            old_holder = "灿灿" if holder == "昭昭" else "昭昭"
            vb = re.sub(
                r"[^。；]*水壶[^。；]*尚未交接[^。；]*",
                f"水壶已在{holder}手中",
                vb,
            )
            vb = vb.replace("水壶悬在中间", f"水壶已在{holder}手中")
            vb = re.sub(
                rf"{old_holder}[^。；]*?(?:捧着水壶|递向|递给|递出|接过)[^。；]*",
                "",
                vb,
            )
            vb = re.sub(
                rf"{holder}[^。；]*?(?:准备接过|伸出手接过)[^。；]*",
                f"{holder}双手握住水壶",
                vb,
            )
        if "昭昭" in vb and "灿灿" in vb:
            vb = re.sub(
                r"画面左边是灿灿[，,，]*右边是昭昭",
                "画面左边是昭昭，右边是灿灿",
                vb,
            )
        if has_tray and "托盘" not in vb:
            vb = "托盘在花盆正下方，" + vb
