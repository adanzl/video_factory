...
_DAILY_VISUAL_SUBJECTS_RULE = (
    ...
    "expression 禁止写任何嘴部姿态词（撇嘴/抿嘴/嘟嘴/嘟起嘴/鼓腮/咬唇/噘嘴/咧嘴/大笑/张嘴/吐舌等），"
    "情绪只能靠眉眼表达（皱眉/瞪眼/眯眼/目光/眼尾/眉心），避免与系统注入口型冲突；"
    ...
    "画面必须三/两人全身同框完整可见；"
    "禁止写走向/走进/转身/背对/离开/出门/进屋/向房间/向门口/出画/入画等位移动词，"
    "改成静态站位（站着/立在/背手站），避免转身/背影/出画破坏同框；"
    "妈妈入镜时必须作为独立完整主体写清（如「妈妈站在门框旁，全身，正面朝镜头」），"
    "禁止把妈妈写成「旁观/站在门内」等易被省略的弱表述；"
    "每个入画角色都写清全身可见与朝向，不得只露半身/头/背影。"
    "【安全】儿童角色不得持真实刀具/锐器；剧情涉及刀时统一写「塑料蛋糕刀」，"
