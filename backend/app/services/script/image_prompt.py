...
_IMAGE_PROMPT_MOTION_TAIL_DAILY_KEYFRAME = (
    "【keyframe / 图生视频】120-200字，上限 600 字，按以下格式逐项输出："
    "1) 角色定位：根据 visual_brief 写明「画面左边是X，右边是Y」；"
    "2) 微动作：每人 1 个可见微动作，须含部位+幅度+时长"
    "（如「右手食指微微向下点动约2厘米后停止」「肩膀轻轻耸起约3厘米后定格」）；"
    "3) 逐位锁表情：每人独立一行，写明表情+情绪备注"
    "（如「灿灿瞪圆眼睛嘴巴大张（惊讶质问状），不微笑」"
    "「昭昭撇着嘴角耸肩（无辜状），表情不变」）；"
    "4) 锁外形：「服装发型稳定，身高比例（X比Y矮半个头）不变」；"
    "5) 末尾：「镜头固定，不推近不拉远，画面只有人物和场景，无任何文字叠加」。"
    "禁止镜头推近/推进/拉远/变焦/放大构图；禁止只写动作不锁表情；"
    "禁止大位移换位、全身换姿势、多人齐跑、抽象光效；禁止用「表情不变」代替逐位描述。"
    "【纯净动作】微动作只描述部位+幅度+时长，禁止加「指向XX」「伸向XX」「碰XX」等情景前缀。"
    "正例："
    "画面左边是妈妈，右边是昭昭。"
    "妈妈右手微微前推约2厘米持续0.5秒后定格，面部表情保持严肃与静图一致不微笑；"
    "昭昭侧脸微抬约1厘米持续1秒后静止，撇嘴表情不变。"
    "服装发型稳定，身高比例（昭昭比妈妈矮半个头）不变。"
    "镜头固定，不推近不拉远，画面只有人物和场景，无任何文字叠加」。"
    "反例：……镜头极缓推近。（关键帧禁止运镜放大，易裁脸）"
