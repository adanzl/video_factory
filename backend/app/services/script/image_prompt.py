...
_IMAGE_PROMPT_MOTION_TAIL_DAILY_KEYFRAME = (
    "【keyframe / 图生视频】120-200字，上限 600 字，按 Agnes I2V 写法："
    "先写 1-2 个可见微动作，每个须写明部位、幅度与时长"
    "（如指尖微微前伸约2厘米持续0.5秒后定格、肩膀轻轻耸起约3厘米持续1秒再放下），"
    "再逐一锁定每位出场角色的面部表情与静图一致（不微笑不大笑），"
    "并锁服装发型五官、身高比例、其余姿势；"
    "末尾写「镜头固定，不推近不拉远」。"
    "【长度】若不足 120 字，须补充第 2 个微动作或环境微动（光影/窗帘/衣物边缘），"
    "但不得仅写环境微动而无人物动作。"
    "禁止镜头推近/推进/拉远/变焦/放大构图；禁止只写动作不锁表情；"
    "禁止大位移换位、全身换姿势、多人齐跑、抽象光效。"
    "正例：妈妈右手微微前推约2厘米持续0.5秒后定格，面部表情保持严肃与静图一致不微笑，"
    "昭昭侧脸微抬约1厘米持续1秒后静止，五官表情不变，服装发型稳定，镜头固定不推近不拉远。"
