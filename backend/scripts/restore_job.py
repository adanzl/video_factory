...
def get_job_log_script(conn: sqlite3.Connection, job_id: int) -> str | None:
    ...
    row = conn.execute("SELECT message FROM job_log WHERE job_id=? AND message LIKE 'script_json=%' ORDER BY id DESC LIMIT 1", (job_id,)).fetchone()
    if not row:
        return None
    raw = row[0]
...
def restore_full(job_id: int) -> None:
    ...
    conn.execute("UPDATE video_job SET script_json=?, status='pending', stage='segment', error_message=NULL WHERE id=?", (json.dumps(script, ensure_ascii=False), job_id))
    ...
    for seg in segments:
        ...
        conn.execute('INSERT INTO video_segment (job_id, segment_index, text, image_prompt, motion_prompt, visual_mode, duration_sec, sd15_prompt_en, image_path, clip_path, status) VALUES (?,?,?,?,?,?,?,?,?,?,?)', (job_id, idx, seg.get('text'), seg.get('image_prompt'), seg.get('motion_prompt'), seg.get('visual_mode'), seg.get('duration_sec'), seg.get('sd15_prompt_en'), None, None, 'pending'))
    ...
    if wan_dir.exists():
        ...
        for r in rows:
            ...
            if Path(p).exists():
                conn.execute('UPDATE video_segment SET clip_path=? WHERE job_id=? AND segment_index=?', (p, job_id, idx))
                c += 1
        print(f'     clip_path restored ({c}/{len(rows)})')
    else:
        print('[4/6] segments_wan not found, skipping')
    s = str(media)
    conn.execute('UPDATE video_job SET audio_path=?, subtitle_path=?, intro_path=?, final_path=? WHERE id=?', (f'{s}/audio/narration.mp3', f'{s}/audio/subtitles.srt', f'{s}/intro.mp4', f'{s}/final_wan.mp4', job_id))
    rows = conn.execute('SELECT segment_index FROM video_segment WHERE job_id=?', (job_id,)).fetchall()
    img_cnt = 0
    for r in rows:
        idx = r[0]
        ...
        if Path(ip).exists():
            conn.execute('UPDATE video_segment SET image_path=? WHERE job_id=? AND segment_index=?', (ip, job_id, idx))
            img_cnt += 1
...
def restore_media_paths(job_id: int, *, dry_run: bool=False) -> dict:
    ...
    for row in rows:
        ...
        if fields:
            ...
            conn.execute(f"UPDATE video_segment SET {', '.join(parts)} WHERE id = ?", values)
    if not dry_run:
        conn.commit()
    conn.close()
    return {'job_id': job_id, 'segments': len(rows), 'updated_images': updated_images, 'updated_clips': updated_clips, 'missing_images': missing_images, 'dry_run': dry_run}
