...
OUTPUT = Path(__file__).resolve().parent / "job28_llm_output.txt"
...
prompts = build_material_script_prompts(
    ...
    max_title_length=16, narration_target_words=785, chars_per_sec=cps,
)

client = DeepSeekClient()
data, finish = client._chat_json(prompts["system"], prompts["user"])
...
# 每段字数对比预算
lines.append("=" * 70)
lines.append("PER-SEGMENT CHECK")
lines.append("=" * 70)
from app.services.script.board_timeline import _max_chars_for_duration, slot_min_chars, slot_target_chars
segs = data.get("segments") or []
for s in segs:
    idx = s.get("segment_index", "?")
    t = s.get("text", "") or ""
    scene = next((x["chapter"] for x in TIMELINE["scenes"] if x["index"] == idx), "")
    dur = next((x["duration_sec"] for x in TIMELINE["scenes"] if x["index"] == idx), 0)
    mc = _max_chars_for_duration(dur, cps)
    mn = slot_min_chars(mc)
    tc = slot_target_chars(mc)
    ok = "✅" if mn <= len(t) <= mc else ("❌ 太少" if len(t) < mn else "❌ 太多")
    lines.append(f"  seg[{idx}] {scene}: {len(t)}字 预算{mn}-{mc} {ok}")
