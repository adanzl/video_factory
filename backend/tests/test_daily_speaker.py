...
def test_build_image_prompts_daily_keyframe_marks_motion_mode():
    ...
    assert "keyframe" in prompts["user"] or "粗粒度微动作" in prompts["user"]
    assert "禁止自编" in prompts["system"] or "TTS" in prompts["system"]
    assert "说话，同时" in prompts["system"]
    assert "点头" in prompts["system"] or "耸肩" in prompts["system"]
    assert "禁止手指" in prompts["system"] or "手指级" in prompts["system"]
