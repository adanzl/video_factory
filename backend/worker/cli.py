...
WORKER_LOG = setup_logging(log_dir=get_settings().log_dir, retention_days=get_settings().log_retention_days)
...
from app.services.segment.clip.subtitle_overlay import rebuild_segment_subtitles
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_p.add_argument('--from-stage', choices=_STAGE_CHOICES, help='从该 stage 起连续跑到结束')
    run_p.add_argument('--only-stage', choices=_STAGE_CHOICES, help='只重跑该 stage 一步（清空该 stage 及下游产物）')
    run_p.add_argument('--segments', help='与 segment 联用：逗号分隔 segment_index，如 1,3')
    run_p.add_argument('--skip-publish', action='store_true', default=None, help='stop before publish (default from env)')
    ...
    sub_p.add_argument('--sentence', type=int, help='only burn one sentence → segments/{N}_test.mp4 (faster)')
    return parser
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if args.title:
        ...
        if args.publish or args.skip_publish is not None:
            with atomic():
                repo_job.update_job(job_id, skip_publish=skip_publish)
    else:
    ...
    try:
        job = job_mgr.run_job_sync(job_id, from_stage=args.from_stage, only_stage=args.only_stage, segment_indices=segment_indices)
...
def cmd_subtitle_test(args: argparse.Namespace) -> int:
    try:
        out = rebuild_segment_subtitles(args.job_id, args.segment, sentence=args.sentence)
