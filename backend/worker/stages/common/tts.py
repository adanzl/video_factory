...
class TTSStage(StageExecutor):
    ...
    def run(self, ctx: JobContext) -> None:
        ...
        with atomic():
            ...
            repo_job_log.append_log(ctx.job['id'], self.name, f'audio {result.duration_sec:.1f}s, segments={len(result.segment_durations)}, cues={len(result.subtitle_cues)}, billing_chars={result.total_characters}, lufs={loudness.integrated_lufs}, max_silence={silence.max_gap_sec:.2f}s')
            apply_quality_checks(ctx.job['id'], self.name, {'tts': check_tts_audio(result.audio_path, result.duration_sec, subtitle_cues=result.subtitle_cues, segments=segments, loudness=loudness, silence=silence, min_duration_sec=min_audio_dur)}, existing_report=job.get('quality_report'))
