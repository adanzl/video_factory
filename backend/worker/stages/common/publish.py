    ...
    def run(self, ctx: JobContext) -> None:
        ...
        with atomic():
            ...
            if not need_description and (not need_tags):
                repo_job_log.append_log(job_id, self.name, 'publish meta ready; manual publish: copy description/tags and download cover/final')
                return
        ...
        if need_description:
            try:
                updated['video_description'] = llm_mgr.generate_video_description(title, narration, content_style=content_style)
                notes.append('video_description generated')
            except Exception as exc:
                warn_logs.append(f'video description failed: {exc}')
        if need_tags:
            try:
                updated['tags'] = llm_mgr.generate_tags(title, narration, content_style=content_style)
        ...
        with atomic():
            ...
            if notes:
                ...
                repo_job_log.append_log(job_id, self.name, '; '.join(notes) + '; manual publish: copy description/tags and download cover/final')
            else:
                repo_job_log.append_log(job_id, self.name, 'publish meta generation skipped; manual publish: copy description and download cover/final')
