    ...
    for attempt in range(1, _TTS_MAX_RETRIES + 1):
        try:
            ...
            if attempt < _TTS_MAX_RETRIES:
                ...
                logger.warning('tts attempt %d/%d failed (voice=%s): %s — retrying in %.1fs', attempt, _TTS_MAX_RETRIES, voice, exc, delay)
                time.sleep(delay)
            else:
...
def _synthesize_segment_dialogue(seg: dict, clips_dir: Path, ext: str, speaker_configs: dict[str, dict], phrase_gap_sec: float, *, job_id: int | None=None) -> _SegResult:
    ...
    for line_idx, line in enumerate(dialogue):
        ...
        logger.info('tts segment %d line %d speaker=%s voice=%s rate=%.2f text_chars=%d text=%s', seg_index, line_idx, speaker, voice, rate, len(text), text)
        ...
        run_ffmpeg(['ffmpeg', '-y', '-hide_banner', '-i', str(wav_clip), '-c:a', 'libmp3lame', '-q:a', '2', str(line_clip)])
        wav_clip.unlink(missing_ok=True)
        line_duration = probe_duration(line_clip)
        seg_cues.append(SubtitleCue(segment_index=seg_index, text=text, duration_sec=line_duration))
    ...
    if cue_total > 0 and abs(cue_total - seg_duration) > 0.01:
        ...
        logger.debug('tts segment %d cue scale=%.6f (sum=%.3f -> actual=%.3f)', seg_index, scale, cue_total, seg_duration)
    logger.info('tts segment %d done duration=%.2fs cues=%d', seg_index, seg_duration, len(seg_cues))
    return _SegResult(seg_index=seg_index, clip_path=seg_clip, cues=seg_cues, duration=seg_duration, chars=total_chars)
...
class DailyTtsStage(StageExecutor):
    ...
    def run(self, ctx: JobContext) -> None:
        ...
        if cue_total > 0 and abs(cue_total - final_duration) > 0.01:
            ...
            for cue in result['subtitle_cues']:
                cue.duration_sec *= scale
            logger.info('tts post-norm cue scale=%.6f (%.3fs -> %.3fs)', scale, cue_total, final_duration)
            result['subtitle_path'].write_text(build_srt_from_cues(result['subtitle_cues']), encoding='utf-8')
    ...
    def _synthesize_multi_speaker(self, segments: list[dict], output_dir: Path, speaker_configs: dict[str, dict], phrase_gap_sec: float=_DEFAULT_PHRASE_GAP_SEC) -> dict:
        ...
        logger.info('tts start: segments=%d phrase_gap=%.2fs speakers=%s max_workers=%d', len(segments), phrase_gap_sec, list(speaker_configs.keys()), max_workers)

        def _run_seg(seg: dict) -> _SegResult:
            return _synthesize_segment_dialogue(seg, clips_dir, ext, speaker_configs, phrase_gap_sec, job_id=self._job_id)
        ...
        if cue_total > 0 and abs(cue_total - total_duration) > 0.01:
            ...
            for cue in all_subtitle_cues:
                cue.duration_sec *= scale
            logger.info('tts final cue scale=%.6f (%.3fs -> %.3fs)', scale, cue_total, total_duration)
        ...
        logger.info('daily tts done duration=%.2fs cues=%s billing_chars=%s', total_duration, len(all_subtitle_cues), total_chars)
        return {'audio_path': audio_path, 'subtitle_path': subtitle_path, 'subtitle_cues_path': cues_path, 'duration_sec': total_duration, 'segment_durations': segment_durations, 'subtitle_cues': all_subtitle_cues, 'usage_summary': {'total_characters': total_chars}}

    def _fallback_single_voice(self, first_seg: dict, all_segments: list[dict], output_dir: Path, speaker_configs: dict) -> dict:
        """回退到单角色合成（使用灿灿的声音）。"""
        from app.services.tts.tts_ali import AliTTSClient
        can_config = speaker_configs.get('灿灿', _DEFAULT_SPEAKER_CONFIGS['灿灿'])
        client = AliTTSClient()
        narration = ''
        result = client.synthesize(narration, all_segments, output_dir, voice=can_config.get('voice_id'), speech_rate=can_config.get('speech_rate'))
        return {'audio_path': result.audio_path, 'subtitle_path': result.subtitle_path, 'subtitle_cues_path': result.subtitle_cues_path, 'duration_sec': result.duration_sec, 'segment_durations': result.segment_durations, 'subtitle_cues': result.subtitle_cues, 'usage_summary': result.usage_summary()}
