...
> **首次实施**：样片验证与 CLI 试运行见 [§13](#13-mvp-试运行)。
...
| **分发边界** | 生产一次、成片多投；**B 站**为激励与 §11 回流主站；其他平台见 [§4.8](#48-多平台分发) |
...
| 5 | 分镜出图 | 9:16 插画 ×6～8 | **ImageProvider**（默认通义万相文生图） | API | **低** |
...
> **分工**：文案与质检主模型 **DeepSeek-V4-Flash**；画面对齐 **Qwen-VL**；默认画面成本在 **文生图**（~0.2 元/张）；**可灵 std** 仅升级开关开启时产生费用；不按电费/存储计成本。
...
| 项 | 标准 |
| --- | --- |
| 分镜数量 | 单条 **6–8** 段，与口播段落一一对应 |
| 画幅 | **9:16** 竖屏，单张推荐 **1080×1920** |
| 出图 | **ImageProvider** 批量文生图（默认通义万相，见 §6.7） |
...
流水线在 `ffmpeg` 阶段产出**终态成片**（9:16、片头+正文、4:20–5:20、带字幕），此后分发与生产解耦。`publish` 阶段为**多适配器**：同一 MP4 + 封面依次（或并行）调用各平台 API，失败仅重试该平台，不回到画面生产。

| 平台 | 开放投稿 API（概况） | 典型门槛 | 默认策略 |
| --- | --- | --- | --- |
| **B 站** | 开放平台上传 + 元数据 | 创作者号、应用审核 | **必接**；Worker 自动发；§11 回流主站 |
...
│   ├── scripts/
│   │   └── init_db.py           # 一次性：python -m scripts.init_db
│   ├── worker/
...
| **API 规范** | flask-smorest 或 Flask-RESTX | — | OpenAPI 文档 |
...
| **分镜出图** | ImageProvider（默认万相文生图） | `DASHSCOPE_API_KEY` | ~0.2 元/张，见 §9 |
| **动态升级** | VideoProvider（可灵 std，可选） | `KLING_API_KEY` | 默认关闭，见 §9 |
...
| `backend/app/repositories/` | `connection.py`、`schema.py`、`job_repo` 等 | 连接 + 建表（Python 内 SQL）+ 业务 CRUD；返回 `dict` / 轻量 dataclass |
...
# scripts/init_db.py（示意）
if __name__ == "__main__":
    with get_connection() as conn:
        apply_schema(conn)
```

**约定**：部署新机 / 本地首次搭环境时跑 **一次** `init_db`；日常启 API + Worker **不再碰** schema。
...
├── base.py              # ImageProvider / VideoProvider 协议
├── registry.py          # 按配置实例化 provider
├── image_wan.py         # 默认：通义万相文生图（百炼）
├── video_kling.py       # 可选：可灵 std
├── motion.py            # FFmpeg 静图→视频片段（Ken Burns 等）
├── host.py              # 讲解人叠图（片头/封面/PiP）
├── cover.py             # 封面流水线
└── intro.py             # 片头流水线
```
...
| 接口 | 职责 | 默认实现 |
| --- | --- | --- |
| **ImageProvider** | `generate(prompt, size, style_ref) → image_path` | `wan_t2i`（万相文生图） |
...
| 变量 | 默认 | 说明 |
| --- | --- | --- |
| `IMAGE_PROVIDER` | `wan_t2i` | 分镜插画 + 封面底图 |
...
| **成本与数据** | 月出图张数、可灵段数（若有）、Token/字数、单条成本、播放收益（§9、§11） |
...
| 分镜出图 | ImageProvider | **7 张** + 封面 1 张 | 默认万相 ~0.2 元/张 |
| 动态升级（可选） | VideoProvider 可灵 std | 0～7 次视频 | 默认 **0** |
| 封面片头 | LLM 构图 + FFmpeg | 封面 1 + 片头 1 | 轻量 |
| 配音字幕 | CosyVoice API | ~1100 字 + 时间轴 | 单次调用 |
| 文本质检 | V4-Flash Thinking ×1 | ~5k in / ~2k out | 合并三轮为一次终审 |
| 画面质检 | Qwen-VL ×8 帧 | 7 分镜 + 1 封面 | 可对可疑帧二检 |
| 音频质检 | 规则检测 | 1 条音轨 | 无 ASR |
| 回流 | V4-Flash（可选） | 日批 | 可忽略单条摊销 |

重制系数：二级 +1～3 张插画 +1～2 VL；三级 +全文案 LLM +8 张图。若开启可灵升级，额外按段计费。API 预算按 **1.3 倍** Token、**1.25 倍** 出图预留。
...
| **默认模型** | 通义万相文生图（百炼），约 **0.2 元/张**（[万相定价](https://help.aliyun.com/zh/model-studio/wanx-video-generation-billing) 以控制台为准） |
| **单条用量** | 7 张分镜 + 1 张封面 = **8 张**；含重制预留 **×1.25** ≈ **10 张/条** |
...
### 9.1 成本结构总览（月产 15 条、默认静图、一次通过率 75%、含 1.25× 出图重制）

| 成本项 | 计费方式 | 月金额（估） | 占比 |
| --- | --- | --- | --- |
| 文生图（8 张×15 条×1.25） | ~0.2 元/张 | **~30 元** | **~55%** |
| DeepSeek V4-Flash | 按 Token | **3～8 元** | ~10% |
| CosyVoice TTS API | 按字数 | **5～12 元** | ~20% |
| Qwen2.5-VL-7B | 按次/Token | **1～3 元** | ~5% |
| FFmpeg / YOLO | 自建 | **0** | — |
| 可灵 std（**升级开启**，7 段全用） | 1.5 元/5s | **+~197 元** | 可选叠加 |

**月现金合计（默认静图 · 无可灵）**：约 **39～53 元**（中性 **~46 元**）。
**若全开可灵 std 升级**：在上表基础上 **+~197 元** ≈ **~243 元/月**（15 条）。
...
| 服务 | 单条用量 | 单条成本（估） |
| --- | --- | --- |
| Qwen-VL 画面对齐 | 8 帧 | **0.06～0.18 元** |
| CosyVoice 配音+字幕 | ~1100 字 | **0.3～0.8 元** |
| 万相文生图 | 8 张×1.25 | **~2.0 元** |

**单条 API 合计（默认静图）**：约 **2.5～3.2 元**。

### 9.3 单条视频全成本

基准：8 张图/条（含封面），含 **1.25×** 重制系数。

| 档位 | 画面成本 | + LLM/TTS/VL | **单条合计** |
| --- | --- | --- | --- |
| **默认静图** | 8×0.2×1.25 ≈ **2.0 元** | ~0.7 元 | **~2.7 元** |
| **+ 可灵 std 全段** | +7×1.5×1.25 ≈ **13.1 元** | ~0.7 元 | **~13.8 元** |
| **+ 可灵 std 2 段**（关键镜头） | +2×1.5 ≈ **3.0 元** | ~0.7 元 | **~5.7 元** |

#### 月预算（默认静图）

| 月产量 | 出图张数（×8×1.25） | 画面月费 | 含其他 API 月合计（估） |
| --- | --- | --- | --- |
| 10 条 | 100 张 | **~20 元** | **~30 元** |
| 15 条 | 150 张 | **~30 元** | **~46 元** |
| 20 条 | 200 张 | **~40 元** | **~61 元** |

#### 可灵 std 升级叠加（仅开关开启时）

| 月产量 | 若 7 段/条全用 std | 叠加月费 |
| --- | --- | --- |
| 15 条 | 131 段 ×1.5 元 | **+~197 元** |
...
例：中性 **默认静图** 月现金 **~46 元** → 46 ÷ 2.3 × 1000 ≈ **2.0 万次/月**（全账号播放加总）。

#### 情景表（默认静图；括号内为 **全开可灵 std 升级**）

| 情景 | 月产量 | 月现金成本 | 单条成本 | 盈亏平衡（月总播放） |
| --- | --- | --- | --- | --- |
| 保守 | 10 条 | **~30 元**（~223） | **~3 元**（~22） | **~1.3 万**（~9.7 万） |
| 中性 | 15 条 | **~46 元**（~243） | **~3 元**（~16） | **~2.0 万**（~10.6 万） |
| 乐观 | 20 条 | **~61 元**（~258） | **~3 元**（~13） | **~2.7 万**（~11.2 万） |

- **月现金**：默认路径下出图 + TTS 为主；可灵仅升级开关开启时叠加。
- **单条成本**：默认 **~2.7 元**；单条 **~1200 次播放**即可接近覆盖制作费。
- **盈亏平衡**：15 条合计 **~2 万月播放**即可打平（默认可灵关）。

#### 单条播放够不够回本？

默认静图单条约 **2.7 元**。若某条播放 **3000 次**：

```text
该条激励收入 ≈ 3000 ÷ 1000 × 2.3 ≈ 6.9 元  >  2.7 元
```

单条 **~1200 次**即可接近回本；账号打平仍看 §9.4 情景表**月总播放**。

升级 **V4-Pro** 仅在三级回退终审时触发，月增 **< 10 元**，对上表影响很小。

### 9.5 重制与失败成本

| 质检结果 | 额外 API | 额外画面（默认静图） |
| --- | --- | --- |
| 一级通过 | 0 | 0 |
| 二级（重 2 张插画） | VL ×2，+~0.1 元 | **+0.4 元**（2×0.2） |
| 三级（文案重做） | LLM 全流程 +~0.15 元 | **+2.0 元**（8×0.2，含封面） |

若该段为可灵升级：二级重制 **+3 元**（2×1.5 std）；三级 **+10.5 元**（7×1.5）。

质量优先策略：**宁可多跑 Thinking 与 VL 二检**，静图重 1 张仅 **~0.2 元**，通过率优先。
...
| 类型 | 金额 / 策略 |
| --- | --- |
| **主成本** | 文生图（默认 **~2 元/条** 画面）+ CosyVoice；月 15 条预算 **~40～55 元** |
| **可选成本** | 可灵 std 升级段；超 `KLING_MONTHLY_BUDGET_CNY` 仅暂停可灵、不暂停静图 |
| **可变成本** | DeepSeek + Qwen-VL，约 0.15～0.3 元/条 |
| **配额管控** | Redis `vf:quota:*`（§6.6）；超预算告警或 `vf:kling:paused`；DeepSeek cap **15 元/月**（可选） |
| **成本仪表盘** | Redis 实时计数 + SQLite 任务汇总；出图张数、可灵段数、Token/字数、播放收益 |
...
| **MVP-1** | 样片满意后 CLI 批量约 **15 条**，看成本与播放 | Worker CLI 循环 / 简单脚本 |
...
| 步骤 | 命令 / 动作 |
| --- | --- |
| 0. 建表（一次性） | `python -m scripts.init_db` |
| 1. 配 `.env` | DeepSeek、百炼（万相+Qwen-VL）、CosyVoice；B 站 Key **样片可不配** |
| 2. 跑一条样片 | 见下方 **CLI 参数**；默认 **`--skip-publish`**，只产出本地成片 |
| 3. 看进度 | 查 SQLite `video_job` / `job_log`（§13.2） |
...
| **MVP 通过后** | 接入管理后台与定时回流（§7、§11） | 再按需接 1～2 个平台 `PublishAdapter` |
...
- [ ] `python -m scripts.init_db` 后，`python -m worker run --title "…" --skip-publish` 可跑通至 `final.mp4`。
- [ ] 无需启动 Flask / `frontend/`。
- [ ] `video_job` / `job_log` 可查到 stage 推进与错误信息。
- [ ] 样片时长约 **4:20–5:20**，画面为静图动效默认路径。

#### MVP-1（批量约 15 条）

- [ ] CLI 可连续投递约 **15 条** 并统计单条成本（§9）。
...
- **外部依赖**：百炼文生图 / CosyVoice / DeepSeek 可用性；可灵仅为可选升级依赖。
...
| 2 | **阿里云百炼** | 万相文生图 + Qwen-VL | 5、6、8 | `DASHSCOPE_API_KEY` |
...
- #2 [阿里云百炼](https://bailian.console.aliyun.com)（万相文生图 + Qwen-VL；CosyVoice 可同 Key，见 §15.2）
- #3 CosyVoice：§15.2
...
DASHSCOPE_API_KEY=          # 万相文生图 + Qwen-VL
...
1. DeepSeek（可先跑通文案链路）  
2. 百炼：万相文生图 + Qwen-VL + CosyVoice（尽量一家 Key）  
...
| init_db | `scripts/init_db.py` 一次性建表/迁表；**非** API/Worker 启动流程 |
