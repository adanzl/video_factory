...
│       │   ├── database.py sql_exec.py schema.py repo_*.py …
...
| `backend/app/repositories/` | `database.py`、`sql_exec.py`、`schema.py`、`repo_*` 等 | Session/SQL + 建表 + CRUD；返回 `dict` |
...
### 6.5 数据层：Flask-SQLAlchemy Session + 手写 SQL

**选型**：**Flask-SQLAlchemy** 管连接与请求级 Session；**不设 ORM Model**，业务 DML 为手写 SQL（`sql_exec` + `?` 占位）。不设 `db/` 目录、**不设独立 `.sql` 文件**——DDL/DML 写在 Python 里，统一放在 `repositories/`。

```text
app/repositories/
├── db_obj.py          # SQLAlchemy() 单例
├── database.py        # init_database、SQLite PRAGMA、teardown
├── sql_exec.py        # execute/fetch、atomic()
├── schema.py          # 建表/迁表（一次性手动执行，见 §6.5）
├── repo_job.py
├── repo_segment.py
├── repo_title.py
├── repo_job_log.py
...
| 文件 | 职责 |
| --- | --- |
| `database.py` | 绑定 `SQLALCHEMY_DATABASE_URI`；**不**在启动时建表 |
| `sql_exec.py` | 读写 SQL；多步写包 `atomic()`；禁止在事务内调 LLM |
| `schema.py` | 建表/补列；**仅一次性手动执行** |
| `repo_*.py` | 业务 CRUD；`core` / worker **只调这里** |
| `app/core/` | 业务规则；**不出现** SQL 字符串 |

Repository 示例

```python
# repositories/repo_job.py
def claim_next_pending() -> dict | None:
    """Worker 取一条 pending job（可与 Redis 锁配合）"""
    ...

def update_stage(job_id: int, stage: str, status: str) -> None:
    ...
```
...
成片路径固定：`data/media/{job_id}/final.mp4`（见 [§6.5](#65-数据层flask-sqlalchemy-session--手写-sql) 产物路径约定）。
...
| 4 | **B 站创作者号** | 浏览器登录投稿 | 10 | `BILI_USERNAME` |
...
> **注意（2026-08-05 实测）**：CosyVoice v3.5-flash 复刻音色上加自然语言
> `instruction`（开心/生气/悲伤等）虽能控制情绪节奏，但会**让音色漂移成
> 另一人**，复刻音色一致性直接丢失。结论：复刻音色**不加情绪指令**，情绪
> 靠对白文案表达。`TTS_INSTRUCTION` / `VOICE_INSTRUCTIONS` 保持默认空。
> 测试脚本：`scripts/test_cosyvoice_emotion.py`；产物
